...
class Network {
	...
	step( { targets } ) {
		...
		for ( let i = 0, l = this.#connectedNeurons.length; i < l; i++ ) {
			...
			if( this.#connectedNeurons[i].affects.direction ) {
				this.direction += this.#connectedNeurons[i].value;
			}
			if( this.#connectedNeurons[i].affects.speed ) {
				this.speed += this.#connectedNeurons[i].value;
			}
		}

		moveX += this.speed * Math.cos( this.direction );
		moveY += this.speed * Math.sin( this.direction );
